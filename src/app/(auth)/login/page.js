...
import { BYPASS_AUTH } from "@/utils/bypassAuth";
...
const LoginPage = () => {
  ...
  const handleBypassDirect = () => {
    ...
    toast.success("Bypass Mode: Logged in successfully!");
    router.push("/teamdetails");
  };

  const onSubmit = async (data) => {
    // TEMPORARY BYPASS: Set BYPASS_AUTH to false in src/utils/bypassAuth.js to restore Firebase auth.
    if (BYPASS_AUTH) {
      if (loginDemoUser) loginDemoUser();
      toast.success("Bypass Mode: Logged in successfully!");
      router.push("/teamdetails");
        ...
        {BYPASS_AUTH && (
          <div className="pt-2 border-t border-white/20">
            <button
              ...
              className="w-full py-2.5 px-4 rounded-md font-semibold text-darkgreen bg-lightgreen hover:bg-green transition-all shadow-md flex items-center justify-center gap-2"
            >
              <span>⚡</span> Developer Bypass: Quick Login
            </button>
            <p className="text-xs text-center text-beige/50 mt-2">
              Temporary auth bypass active for frontend testing
            </p>
          </div>
        )}
      </div>
